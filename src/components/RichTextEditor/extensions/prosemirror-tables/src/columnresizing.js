...
function handleMouseDown(view, event, cellMinWidth) {
  ...
  const nextPos =
    ...
    view.state.doc.resolve(pluginState.activeHandle).nodeAfter.nodeSize
  let nextCell = view.state.doc.nodeAt(nextPos)
  let width = currentColWidth(view, pluginState.activeHandle, cell.attrs)
  let nextWidth = currentColWidth(view, nextPos, {
    ...nextCell.attrs,
    colwidth: [...(nextCell.attrs.colwidth ?? [])].reverse(),
  })
  view.dispatch(
    view.state.tr.setMeta(key, {
      setDragging: {
        ...
        startWidth: width,
        startWidthNext: nextWidth,
        activeHandleNext: nextPos,
      },
    })
  )
  ...
  function finish(event) {
    ...
    if (pluginState.dragging) {
      ...
      updateColumnWidth(
        ...
        pluginState.activeHandle +
          view.state.doc.resolve(pluginState.activeHandle).nodeAfter.nodeSize,
        pluginState.dragging.startWidth +
          pluginState.dragging.startWidthNext -
          draggedWidth(pluginState.dragging, event, cellMinWidth),
        false
      )
  ...
  function move(event) {
    ...
    let pluginState = key.getState(view.state)
    view.domAtPos(pluginState.activeHandle + 1).node.style.cursor = 'col-resize'
...
function draggedWidth(dragging, event, cellMinWidth) {
  ...
  if (offset > dragging.startWidthNext - cellMinWidth - 10)
    offset = dragging.startWidthNext - cellMinWidth - 10
...
function updateColumnWidth(view, cell, width, leftCell = true) {
  let $cell = view.state.doc.resolve(cell)
  let table = $cell.node(-1),
    map = TableMap.get(table),
    start = $cell.start(-1)
  let col =
    ...
    (leftCell ? $cell.nodeAfter.attrs.colspan - 1 : 0)
...
function displayColumnWidth(view, cell, width, cellMinWidth) {
  ...
  const nextWidth =
    ...
    pluginState.dragging.startWidthNext -
    width
  updateColumns(table, dom.firstChild, dom, cellMinWidth, col, width, nextWidth)
}
