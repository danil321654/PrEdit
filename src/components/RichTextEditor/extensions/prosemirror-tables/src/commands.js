...
export function addColumn(tr, { map, tableStart, table }, col) {
  ...
  for (let row = 0; row < map.height; row++) {
    ...
    if (col > 0 && col < map.width && map.map[index - 1] == map.map[index]) {
      ...
      let previousCell = table.nodeAt(pos)
      let colInd = 0
      let extraPos = pos
      while (
        (!previousCell ||
          !previousCell.attrs?.colwidth ||
          +previousCell.attrs?.colwidth?.[0] < CELL_MIN_WIDTH * 2) &&
        colInd < map.width
      ) {
        extraPos = map.positionAt(row, colInd, table)
        previousCell = table.nodeAt(extraPos)
        colInd++
      }
      if (
        previousCell.attrs?.colwidth &&
        ...
        tr.setNodeMarkup(
          tr.mapping.map(tableStart + extraPos),
          null,
          setAttr(previousCell.attrs, 'colwidth', [
            previousCell.attrs.colwidth[0] - CELL_MIN_WIDTH,
          ]),
        )
      } else {
        return tr
      }
      let baseAttrs = previousCell?.attrs
      baseAttrs = setAttr(baseAttrs, 'colwidth', [CELL_MIN_WIDTH])
      baseAttrs = setAttr(baseAttrs, 'colspan', 1)
      baseAttrs = setAttr(baseAttrs, 'rowspan', 1)
      let copy = type.createAndFill(
        setAttr(baseAttrs, 'colwidth', [CELL_MIN_WIDTH]),
      )
      tr.insert(tr.mapping.map(tableStart + pos), copy)
    }
  }
  return tr
}
...
export function removeColumn(tr, { map, table, tableStart }, col) {
  ...
  for (let row = 0; row < map.height; ) {
    ...
    if (
      ...
      let addToLeft = col + 1 == map.width
      let index2 = row * map.width + col + (addToLeft ? -1 : 1),
        pos2 = map.map[index2]
      const deletedNode = table.nodeAt(pos)
      const nextNode = table.nodeAt(pos2)

      if (nextNode.attrs.colwidth) {
        tr.setNodeMarkup(
          tr.mapping.map(tableStart + pos2),
          null,
          setAttr(nextNode.attrs, 'colwidth', [
            +nextNode.attrs.colwidth[0] +
              +deletedNode.attrs.colwidth.reduce((x, y) => +x + +y, 0),
          ]),
        )
      }
...
export function addRow(tr, { map, tableStart, table }, row) {
  ...
  for (let col = 0, index = map.width * row; col < map.width; col++, index++) {
    ...
    if (
      ...
      const cell = table.nodeAt(map.map[index + refRow * map.width])
      let type =
        refRow == null ? tableNodeTypes(table.type.schema).cell : cell.type
      cells.push(type.createAndFill(cell.attrs))
    }
  }
...
export function mergeCells(state, dispatch) {
  ...
  if (dispatch) {
    ...
    for (let row = rect.top; row < rect.bottom; row++) {
      for (let col = rect.left; col < rect.right; col++) {
        ...
        if (mergedPos == null) {
          ...
          if (row === rect.top && cell.attrs.colwidth) {
            deletedCellColwidth = [
              ...deletedCellColwidth,
              ...cell.attrs.colwidth,
            ]
    ...
    tr.setNodeMarkup(
      tr.mapping.map(mergedPos + rect.tableStart),
      null,
      setAttr(
        setAttr(
          setAttr(mergedCell.attrs, 'colwidth', [
            ...mergedCell.attrs.colwidth,
            ...deletedCellColwidth,
          ]),
          'colspan',
          mergedCell.attrs.colwidth.length + deletedCellColwidth.length,
        ),
        'rowspan',
