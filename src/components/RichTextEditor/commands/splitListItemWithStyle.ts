...
import { RawCommands, getNodeType, ExtensionAttribute } from '@tiptap/core'
import {
  NodeType,
...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    splitListItemWithStyle: {
       ...
       */
      splitListItemWithStyle: (typeOrName: string | NodeType) => ReturnType
    }
  }
}

export const splitListItemWithStyle: RawCommands['splitListItem'] =
  typeOrName =>
  ({ tr, state, dispatch, editor }) => {
    const type = getNodeType(typeOrName, state.schema)
    const { $from, $to } = state.selection

    const node = (state.selection as any).node as ProseMirrorNode
    ...
    if (
      ...
      if (dispatch) {
        tr.split($from.pos, 2, types)
          .scrollIntoView()
          .addStoredMark(state.schema.marks.textStyle.create(style))
      }
    }
    return true
  }
